...
class WiFiManager:
    ...
    ### ============= 網路時間同步 NTP & GET HTTP_TIME   ======================
    def sync_time(self):
        """優先使用 NTP 進行時間同步，若失敗則使用 HTTP 備援"""
        if self.tw_ntp():
            return True  # NTP 成功
        #print("NTP 失敗，改用 HTTP 時間同步")
        #return self.get_http_time()
    
    # 增加多個NTP伺服器選項(失敗就會跳下一個嘗試)
    def tw_ntp(self, must=False):
        ntp_servers = [
        ...
        for server in ntp_servers:
            try:
                ...
                #sleep(1)
                sleep(1)  # 隨機等待 1~3 秒，降低被封鎖的風險
            continue  # 不 return False，繼續嘗試下一個伺服器
        print("所有 NTP 伺服器皆無法同步，接下來啟用 HTTP 方式備援時間")
        # 用http做時間同步的備援
        return self.get_http_time()
