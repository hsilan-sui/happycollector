...
class MqttManager:
    ...
    ###測試當前時間是否正確　不正確再繼續觸發取的時間
    def is_valid_time(self):
        import machine
        rtc = machine.RTC()
        year, month, day, _, hour, minute, second, _ = rtc.datetime()

        if year < 2023 or not (1 <= month <= 12) or not (1 <= day <= 31) or not (0 <= hour < 24) or not (0 <= minute < 60) or not (0 <= second < 60):
            print(f"Debugger: [RTC 時間異常] {year}-{month}-{day} {hour}:{minute}:{second}")
            return False

        print(f"Debugger: [RTC 正常]: {year}-{month}-{day} {hour}:{minute}:{second}")
        return True

    #### 發佈getTimeNow請求 |手動請求 
    def request_time(self):
        """ 發布 getTimeNow 請求，請求當前時間 """
