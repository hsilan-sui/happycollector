        ...
        if now_main_state.state == MainStatus.NONE_WIFI:
            ...
            if mq_client_1 is not None:
                try:
                    ...
                    ####加入mett時間備援機制###
                    # print(f"Debugger:[手動觸發取得mqtt時間]")
                    # getTimeNow = mqtt_manager.request_time()
                    # print(f"Debugger:[取得mqtt時間]: {getTimeNow}")
                    #print(f"Debugger:[RTC 時間異常] 手動觸發取得mqtt時間")
                    #getTimeNow = mqtt_manager.request_time()
                    # getTimeNow = mqtt_manager.request_time()
                    # print(f"Debugger:[取得mqtt時間]: {getTimeNow}")
                    ####加入判斷 #####
                    if not mqtt_manager.is_valid_time():
                        print(f"Debugger:[RTC 時間異常] 手動觸發取得mqtt時間")
                        getTimeNow = mqtt_manager.request_time()
                        print("Debugger:[取得mqtt時間]")
                    ####加入mett時間備援機制###
