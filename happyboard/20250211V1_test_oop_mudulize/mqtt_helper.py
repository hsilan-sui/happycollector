...
# # 回調訂閱/fota
# def process_fota(data, publish_MQTT_claw_data, claw_1):
#     otafile = 'otalist.dat'
#     try:
...
# ## 使用函式參數傳遞 替代全域變數
# #data, self.publish, self.uart_send_packet_func, self.received_claw_data, self.kind_feiloli_command
# def process_commands(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1, KindFEILOLIcmd=None):
#     command_handlers = {
...
#             if KindFEILOLIcmd:
#                 handler(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1, KindFEILOLIcmd)
#             else:
#                 print(f"Command {command} requires KindFEILOLIcmd but it was not provided.")
#         else:
#             # 這裡是不會需要使用KindFEILOLIcmd 的函式
#             handler(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1)
#     else:
#         print(f"Unknown command: {command}")

# ## 以下針對上述的dict來對應的工具函式

# ## ping <==> handle_ping(data, publish_MQTT_claw_data, uart_send_packet_func, claw)
# def handle_ping(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1):
#     publish_MQTT_claw_data(claw_1, 'commandack-pong')

# ## version
# def handle_version(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1):
#     publish_MQTT_claw_data(claw_1, 'commandack-version')

# ## status ===> 待測試

# # def handle_status(data, publish_MQTT_claw_data, uart_send_packet_func, claw):
# #     publish_MQTT_claw_data(claw, 'commandack-status')

# ## 暖重置
# def handle_claw_reboot(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1, KindFEILOLIcmd):
#     state = data.get('state', '')
#     publish_MQTT_claw_data(claw_1, 'commandack-clawreboot', state)
#     uart_send_packet_func(KindFEILOLIcmd.Send_Machine_reboot)

# ## 啟動遊戲
# def handle_claw_start_game(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1, KindFEILOLIcmd):
#     try:
#         state = data.get('state', '')
...
#         publish_MQTT_claw_data(claw_1, 'commandack-clawstartgame', state)
#         uart_send_packet_func(KindFEILOLIcmd.Send_Starting_once_game, game_data)
#     except Exception as e:
#         print(f"Error handling clawstartgame: {e}")

# ## 遠端帳目清除
# def handle_claw_clean_trans_account(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1, KindFEILOLIcmd):
#     state = data.get('state', '')
#     account = data.get('account', '').split(", ")
#     publish_MQTT_claw_data(claw_1, 'commandack-clawcleantransaccount', state)
#     uart_send_packet_func(KindFEILOLIcmd.Send_Clean_transaction_account, account)

# ## 機台設定
# def handle_claw_machine_setting(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1, KindFEILOLIcmd):
#     setting = data.get('setting', '').strip()
#     valid_settings = ["BasicsettingA", "BasicsettingB", "BasicsettingC", "Clawvoltage", "Motorspeed"]
#     if setting in valid_settings:
#         uart_send_packet_func(KindFEILOLIcmd.Ask_Machine_setting, setting)
#     else:
#         print(f"Invalid machine setting: {setting}")

# ## 檔案相關的commands
# def handle_file_commands(data, publish_MQTT_claw_data, uart_send_packet_func, claw_1):
#     command = data.get('commands')
#     filename = data.get('filename', '')

#     if command == 'fileinfo':
#         publish_MQTT_claw_data(claw_1, 'commandack-fileinfo', filename)
#     elif command == 'fileremove':
#         publish_MQTT_claw_data(claw_1, 'commandack-fileremove', filename)
