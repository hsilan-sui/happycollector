...
class Senko:
    ...
    def _check_all(self):
        ...
        for file in self.files:
            ...
            print(f"debug:[latest_version與local_version]尚未是空字串,記憶體:{gc.mem_free()}")
            latest_version=""
            local_version = ""
            print(f"debug:[latest_version與local_version]已變成空字串,記憶體:{gc.mem_free()}")
            # 釋放記憶體
            del latest_version, local_version
            gc.collect()
            print(f"debug:[gc後的記憶體&del]:{gc.mem_free()}")
        return changes
    # def _check_all(self):
    #     changes = []
