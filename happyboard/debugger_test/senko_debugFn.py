'''
DubugFn這版本的Senko.py 
因為在多處調用micropython.mem_info，所以比較繁重，
但這個比較好找bug
只做測試用，OTA不成功，
就先嘗試使用輕量版的senko_debug 或 senko_print
'''
import urequests
import uhashlib
import gc
...
class Senko:
    ...
    def _get_file(self, url):
        ...
        ### LOG 記憶體 ###
        mem_log('_get_file', '執行發送 HTTP GET 請求.get()')
    ...
    def _check_all(self):
        ...
        ### LOG 記憶體 ###
        mem_log('_check_all', '首行') 
        #gc.collect()
        index_times = 0
        gc.collect()
        ### LOG 記憶體 ###
        mem_log('_check_all', '進入files迴圈前') 
        ...
        for file in self.files:
            ...
            if latest_version is None:
                ...
                mem_log('_check_all', '沒有 latest_version')
                continue
            ...
            try:
                with open(file, "r") as local_file: #讀取 ESP32 本地版本的相同檔案
                    ...
                    mem_log('_check_all', '讀取local_version')
            except:
                local_version = ""
    ...
    def update(self):
        ...
        mem_log('senko.update', '已執行完_check_all，記憶體:')
        ...
        if changes:
            print('Debugger:[senko.update] 檔案已更新清單:{changes}')
            mem_log('senko.update', '檔案清單更新後，記憶體:')
            print('Debugger:[senko.update] 準備回到main.py ===> 執行machine.reset')
            return True
        else:
            #print(f'Debugger[senko.update] 遠端檔案 和本地檔案 一致 無須更新')
            mem_log('senko.update', '遠端和本地檔案 一致，無須更新，將回到main.py，LOG:ESP OTA OK | 記憶體:') 
            return False
